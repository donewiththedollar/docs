...
────────────────┼────────────────────────────
< 1 hour        │ Wait for TP
1-4 hours       │ Tighten TP target
4-8 hours       │ Exit at breakeven if possible
8+ hours        │ Force exit (accept small loss)
```
...
<ParamField path="time_decay.breakeven_after_seconds" type="integer" default="3600">
  Exit at breakeven after this many seconds (1 hour).
</ParamField>

<ParamField path="time_decay.exit_loss_after_seconds" type="integer" default="14400">
  Accept loss exit after this many seconds (4 hours).
</ParamField>

<ParamField path="time_decay.force_exit_after_seconds" type="integer" default="28800">
  Force market exit after this many seconds (8 hours).
</ParamField>
...
```json
{
  ...
  "time_decay": {
    "enabled": true,
    "breakeven_after_seconds": 14400,
    "force_exit_after_seconds": 28800
  }
}
```

### Bybit MM (Recommended for Bybit)
